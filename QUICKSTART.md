...
启停全部收在这一个入口里，双击后是菜单：`[1] 启动服务`、`[2] 停止服务`、`[3] 查看运行状态`、`[0] 退出`。
也可以跳过菜单直接执行 `.\学习搭子.cmd start`（`stop` / `status` 同理）。

**启动**：清理旧实例 → 启动后端（:8000）→ 启动前端（:5173）→ 确认页面可用后自动打开浏览器。

**停止**：释放 8000 / 5173；也可以在网页右上角直接点「停止」。
...
| 想彻底关掉服务 | 双击 `学习搭子.cmd` 选 2，或在页面右上角点「停止」 |
