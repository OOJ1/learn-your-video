...
**双击即启动，不进菜单。** 停止有两种方式：**点网页右上角的「停止」**，或 `.\学习搭子.cmd stop`。

想查看运行状态用 `.\学习搭子.cmd status`；想用交互面板（启动 / 停止 / 状态三选一）用 `.\学习搭子.cmd menu`。
启动会确认页面可用后再打开浏览器；首次进入介绍页还能一键创建带图标的桌面快捷方式（双击即启动）。
...
│   └── public/               # 图标与字体
├── 学习搭子.cmd              # 统一启停入口：双击启动，亦支持 stop / status / menu
