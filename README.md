...
| 文章 | txt / md / pdf 直接解析进上下文，不做向量化，更快 |
...
会自动启动前后端并（确认页面可用后）打开浏览器。停止用 `stop.cmd` / `.\stop.ps1`，
也可以在网页右上角点「停止」；首次进入介绍页还能一键创建带图标的桌面快捷方式。
...
│   └── public/               # 图标与字体
├── start.ps1  / start.cmd    # 一键启动（后台运行，不弹 cmd 窗口）
├── stop.ps1   / stop.cmd     # 一键停止（释放 8000 / 5173）
├── logs/                     # 运行日志（不入库）
